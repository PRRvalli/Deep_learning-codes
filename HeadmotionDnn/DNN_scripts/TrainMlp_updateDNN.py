...
#validation_datafile = sys.argv[7];
...
if(train_flag==1):
 ...
 # early_stopping = EarlyStopping(monitor='val_loss', patience=3)
  history2=model.fit(Xtrain,Ytrain, nb_epoch=int(newDict['nbepochs']), batch_size=int(newDict['batchsize']),verbose=2,validation_split=0.0)#, callbacks=[early_stopping])
...
#  early_stopping = EarlyStopping(monitor='val_loss', patience=3)
  history2=model.fit(Xtrain,Ytrain, nb_epoch=int(newDict['nbepochs']), batch_size=int(newDict['batchsize']),verbose=1, validation_split=0.0)#, callbacks=[early_stopping])
  model.save_weights(update_mdlfile+'/best_model',overwrite=True)

pred = model.predict(Xtrain);
print(numpy.mean((pred-Ytrain)**2))
